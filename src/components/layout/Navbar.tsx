  ...
  return (
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 lg:hidden">
        <div className="mx-2 mb-8 flex items-center justify-around rounded-2xl bg-card/95 backdrop-blur-xl border border-border px-1 py-1.5">
          {NAV_ITEMS.map((item) => {
            const isActive =
              item.href === '/'
                ? pathname === '/'
      ...
      <nav className="fixed left-0 top-0 bottom-0 z-50 hidden lg:flex w-20 flex-col items-center gap-2 bg-card/90 backdrop-blur-xl border-r border-border py-6">
        <Link href="/" className="mb-4">
          <img src="/icons/icon-192.png" alt="ParrotCare" className="w-10 h-10 rounded-xl" />
        </Link>
